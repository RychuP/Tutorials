namespace TollSystem
{
    ...
    class TollPricing
    {
        ...
        static Toll CalculateToll(Vehicle veh) =>
            veh switch
            {
                ...
                Bus b when ((double)b.Riders / (double)b.Capacity) < 0.5d => new Toll(busToll, 2.0m),
                Bus b when ((double)b.Riders / (double)b.Capacity) > 0.9d => new Toll(busToll, -1.0m),
    ...
    class Bus : Vehicle
    {
        ...
        public override string Contents => $"Occupied seats: {((double)Riders / (double)Capacity) * 100:0}%";
    }
}